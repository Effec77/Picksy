...
async function startPriceComparison() {
  ...
  try {
    ...
    // CRITICAL: Log the current product state
    console.log('🔍 DEBUG: currentProduct before check:', currentProduct);
    console.log('🔍 DEBUG: currentProduct type:', typeof currentProduct);
    
    // Create mock product if none exists
    if (!currentProduct) {
      console.warn('⚠️ No currentProduct found, using mock data');
      currentProduct = {
    ...
    if (!currentProduct.title || currentProduct.title === 'undefined' || currentProduct.title === 'null') {
      console.error('❌ Product title is invalid:', currentProduct.title);
      console.error('❌ Full product object:', JSON.stringify(currentProduct, null, 2));
    ...
    console.log('🚀 SIMPLE: Full product:', JSON.stringify(currentProduct, null, 2));
...
// Setup comparison tab functionality - ENHANCED VERSION
function setupComparisonTab() {
  console.log('🔧 Setting up comparison tab...');
  
  // STEP 1: Check for cached comparison results first
  if (currentProduct) {
    restoreCachedComparisonResults(currentProduct);
  }
  ...
  // Setup clear cache button if it exists
  const clearCacheBtn = document.getElementById('clearCacheBtn');
  if (clearCacheBtn) {
    clearCacheBtn.replaceWith(clearCacheBtn.cloneNode(true));
    const newClearBtn = document.getElementById('clearCacheBtn');
    
    newClearBtn.addEventListener('click', async () => {
      console.log('🗑️ Clear cache button clicked!');
      await clearComparisonCache();
    });
    
    console.log('✅ Clear cache button event listener attached');
  }
}

// Clear comparison cache for current product
async function clearComparisonCache() {
  if (!currentProduct) {
  ...
  try {
    ...
    if (typeof PriceComparison === 'undefined') {
      showToast('❌ Price comparison engine not available', 'error');
      return;
    }
    
    await PriceComparison.clearCache(currentProduct);
    
    // Clear the comparison results display
    ...
    // Reset the comparison interface
    const comparisonResults = document.getElementById('comparisonResults');
    ...
    showToast('🗑️ Comparison cache cleared', 'success');
    console.log('✅ Comparison cache cleared successfully');
...
// Restore cached comparison results when extension reopens
async function restoreCachedComparisonResults(product) {
  console.log('🔍 Checking for cached comparison results...');
  
  try {
    // Load price comparison engine if not loaded
    await loadPriceComparisonEngine();
    
    if (typeof PriceComparison === 'undefined') {
      console.warn('⚠️ PriceComparison not available for cache restoration');
      return;
    }
    
    // Get cached display results
    const cachedDisplay = await PriceComparison.getCachedDisplayResults(product);
    
    if (cachedDisplay) {
      console.log('📦 Restoring cached comparison results');
      
      // Display the cached results
      displayComparisonResults({
        originalProduct: product,
        matches: [...cachedDisplay.exactMatches, ...cachedDisplay.similarMatches],
        bestDeal: cachedDisplay.bestDeal,
        savings: cachedDisplay.savings,
        recommendation: cachedDisplay.recommendation,
        analysis: cachedDisplay.analysis
      });
      
      // Show cache info
      const cacheAge = Math.round((Date.now() - cachedDisplay.timestamp) / (1000 * 60));
      const expiresIn = Math.round((cachedDisplay.expiresAt - Date.now()) / (1000 * 60));
      
      showToast(`📦 Restored cached results (${cacheAge}m old, expires in ${expiresIn}m)`, 'info');
      
    } else {
      console.log('📭 No cached comparison results found');
    }
    
  } catch (error) {
    console.error('❌ Failed to restore cached results:', error);
  }
}
